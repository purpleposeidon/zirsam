...
class ValsiParser:
  ...
  def locate_ps(self, start):
    ...
    while 1:
      ...
      if self.bit[locus_ps].accented:
        return locus_ps
      elif self.bit[locus_ps].wordsep:
        found_word_end = True
        break
  ...
  def tokenize(self, count, t_type, start=0):
    ...
    while count:
      ...
      if len(v) and b.wordsep:
        self.config.debug(v, repr(b), self.bit.buffer)
  ...
  def get_token(self):
    ...
    if self.bit[word_end-1].has_C or (self.bit[word_end-1].period and (word_end >= 2 and self.bit[word_end-2].has_C)): #It's a cmene! OMG!
      ...
      if self.bit[0].period:
        ...
        if self.config.dotside:
          self.config.strict("Dotside requires a period in front of cmene (Try --no-dotside)", self.bit[0].position)
    ...
    if self.bit[word_end-1].value == 'y':
      ...
      if didstuff:
        ...
        self.config.warn("Nothing happened when dealing with lerfu", self.bit[0].position)
        self.config.warn(self.bit.buffer)
